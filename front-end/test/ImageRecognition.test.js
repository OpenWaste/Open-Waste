...
describe('CameraTriggerButton Component Tests', () => {
  ...
  it('CameraTriggerButton renders correctly', () => {
    ...
    const tree = render(<CameraTriggerButton camera={cameraMock} uriSetter={() => { }} isPictureTakenSetter={() => { }} predictionTextSetter={() => { }} predictionFetcher={() => { }} />).toJSON();
    expect(tree).toMatchSnapshot();
  });
  ...
  it('Camera snaps a picture upon pressing trigger by calling its helper functions', async () => {
    ...
    expect(predictionTextSetterMock).toHaveBeenCalledWith(predictionResponseMock.prediction)
...
describe('PredictionText Component Tests', () => {
  ...
  it('PredictionText loads text if length > 0', () => {
    ...
    const { queryByTestId } = render(<PredictionText predictionString={predictionText} />)

    expect(queryByTestId("spinner-component")).toBeNull()
    expect(queryByTestId("prediction-text").props.children).toBe(predictionText)
...
describe('PostPictureSnapButtons Component Tests', () => {
  ...
  it('PostPictureSnapButtons renders correctly', () => {
    ...
    const tree = render(<NativeBaseProvider initialWindowMetrics={inset}><PostPictureSnapButtons uriSetter={() => { }} isPictureTakenSetter={() => { }} predictionTextSetter={() => { }} visibilitySetter={() => { }} /></NativeBaseProvider>).toJSON();
    expect(tree).toMatchSnapshot();
